...
# u_3; Recover rate of HR for A
# u_4; Recover rate of HR for B
...

{
  ...
  # Avoid replication rate less then 0
  # Intrinsic death rate is 0.01
  mrates <- function(x, params, t) {
    ...
    return(c(
      S * (ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                  ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0)),                                         # S population growth
      S * (params$dS),                                                                                 # S population death
      S * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                         ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0)) * params$rS_RA,                   # S to A mutation
      S * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                         ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0)) * params$rS_RB,                   # S to B mutation
      S * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                         ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0)) * params$rS_RA * params$rS_RB,    # S to AB mutation
      ...
      RA * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT) > 0, 
                          ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT), 0)) * params$rRA_S,              # RA to S recover
      RA * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT) > 0, 
                          ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT), 0)) * params$rRA_RAB,            # RA to RAB mutation
      ...
      RB *(0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT) > 0, 
                         ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT), 0)) * params$rRB_S,               # RB to S recover
      RB * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT) > 0, 
                          ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT), 0)) * params$rRB_RAB,            # RB to RAB mutation
      ...
      RAB * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT^2) > 0, 
                           ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT^2), 0)) * params$rRAB_RB,         # RAB to RB recover
      RAB * (0.01 + ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT^2) > 0, 
                           ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax*FIT^2), 0)) * params$rRAB_RA          # RAB to RA recover
      
    ))
  }
  
  # Set up bacterial growth and death as deterministic step
