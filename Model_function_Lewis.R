...
# PK-PD model for INC, SAME and SEQ treatments based on Lewis-Thinning-Algorithm in Hybrid format
...
# For the Lewis-Thinning Algorithm in Hybrid format check Nyhoegen et al. 2023 (Lewis et al. (1979) and Kiehl et al. (2004))
...
CS_model <- function(t_half = 2, F_Css_MIC = 1, 
                     ...
                     FIT = 1, Gmin_A = -6, Gmin_B = -6, HILL_A  = 1, HILL_B = 1, u_1 = 10^-6, u_2 = 10^-6, eB0 = 6, RA0=0, RB0 =0 , RAB0 = 0,  Bmax = 9,  V = 1000, n = 100, DT = 1, ST = 24, MIC_S = 1, MIC_R =8, Gmax = 1.5)
...

{
  ...
  # ODE for PD Model as described in SI material of this study 
  PD_mod<- rxode2({
    d/dt(A) = -ke*A   
    d/dt(B) = -ke*B
    d/dt(S) = S * (ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                          ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0) - 
                     (((1 - Gmin_A/Gmax)*(A/MIC_S)^HILL_A/((A/MIC_S)^HILL_A - (Gmin_A/Gmax)))   +
                        ((1 - Gmin_B/Gmax)*(B/MIC_S)^HILL_B/((B/MIC_S)^HILL_B - (Gmin_B/Gmax))))*Gmax)
    d/dt(RA) = RA * (ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                            ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0) * FIT - 
                       (((1 - Gmin_A/Gmax)*(A/MIC_R)^HILL_A/((A/MIC_R)^HILL_A - (Gmin_A/Gmax)))   +
                          ((1 - Gmin_B/Gmax)*(B/MIC_S)^HILL_B/((B/MIC_S)^HILL_B - (Gmin_B/Gmax))))*Gmax*FIT)
    d/dt(RB) = RB * (ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                            ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0) * FIT - 
                       (((1 - Gmin_A/Gmax)*(A/MIC_S)^HILL_A/((A/MIC_S)^HILL_A - (Gmin_A/Gmax)))   +
                          ((1 - Gmin_B/Gmax)*(B/MIC_R)^HILL_B/((B/MIC_R)^HILL_B - (Gmin_B/Gmax))))*Gmax*FIT)
    d/dt(RAB) = RAB * (ifelse(((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax) > 0, 
                              ((1-((S+RA+RB+RAB)/(10^Bmax*V)))*Gmax), 0) * FIT^2 - 
                         (((1 - Gmin_A/Gmax)*(A/MIC_R)^HILL_A/((A/MIC_R)^HILL_A - (Gmin_A/Gmax)))   +
                            ((1 - Gmin_B/Gmax)*(B/MIC_R)^HILL_B/((B/MIC_R)^HILL_B - (Gmin_B/Gmax))))*Gmax*FIT^2)
    
    
  });
  
  
  # PK Model and dDosing regimens and definitions of treatments as given by Aulin et al 2021
  ...
  # We used the Lewis Thinning algorithm (in Hybrid format) instead and incorporated a different PD Model (see above)
  ...
  df_full_CS <-  foreach( ii = 1:n, .combine = "rbind",              # Parallelization
                          ...
                          .options.RNG = 123) %dorng%  {            
                            ...
                            for(i_mod in 1:length(PK_model_list)) {
                              ...
                              # s is maximal time step and used for data record (so that data does not need to be saved every small time step)
                              t = 0
                              s = 1 
                              lambda_s = 0
                              lambda_sup = 0
                              #########
                              # running the simulation until treatment end is reached one time step at the time (i) needed for the stochastic implementation for mutation rate
                              #-------------------
                              while (t < ST*DT) {
                                
                                # analogue to Aulin et al. 2021: check if the number of bacteria can be converted to integer 
                                ...
                                lambda_max  <- c(
                                  S_i * (Gmax + 0.01) * rS_RA * (1-rS_RB)+ 
                                    S_i * (Gmax + 0.01) * rS_RB * (1-rS_RA)+ 
                                    S_i * (Gmax + 0.01) * rS_RB * rS_RA,           
                                  RA_i *  (Gmax*FIT + 0.01) * rRA_RAB,      
                                  RB_i *  (Gmax*FIT + 0.01) * rRB_RAB)
                                ...
                                # Doing GillespieRunnin Thinning algorithm for mutation and ODE for growth and death
                                # if time step is small enough (i.e. tau < 1) we will use the algorithm to determine which mutation will occur 
                                # In case time step gets to large, we will first update the ODE system and check again when the next mutation occurs (so that growth and death event take place regularly)
                                if(tau_PD < 1){
                                  ...
                                  gRB <- gS * FIT
                                  gRAB <- gS * FIT^2
                                  d0  <-  ifelse(((1-((S_i+RA_i+RB_i+RAB_i)/(10^Bmax*V)))*0.01) > 0, 
                                                 ((1-((S_i+RA_i+RB_i+RAB_i)/(10^Bmax*V)))*0.01), 0)
                                  ...
                                  # Initialize lambda_i
                                  lambda_i <- c(                         
                                    S_i * (gS +d0) * rS_RA * (1-rS_RB),           # S to RA mutation
                                    S_i * (gS +d0) * rS_RB * (1-rS_RA),           # S to RB mutation  
                                    S_i * (gS +d0) * rS_RB * rS_RA,                # S to RAB mutation
                                    RA_i * (gRA +d0) * rRA_RAB,                    # RA to RAB mutation                                                   
                                    RB_i * (gRB +d0) * rRB_RAB,                    # RB to RAB mutation                                                    
                                  ...
                                  if (r3 <= lambda_s/lambda_sup){
                                    ...
                                    else if(reaction_first == 4){RAB_i = RAB_i + 1; RA_i = RA_i - 1}
                                    else if(reaction_first == 5){RAB_i = RAB_i + 1; RB_i = RB_i - 1}
                                ...
                                # Doing ODE for growth and death so that change in population due to replication and death gets regularly updated 
                                # if tau >= 1
                                if(tau_PD >= 1) {
